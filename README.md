...
This pipeline uses 3 family models: 2 linear models, 2 non-linear and 2 neural networks. Each family has a version which incorporates an exogenous variable and one which doesn't. The exogenous variable used are the wasserstein distance between persistance diagrams of subsequent time windows of the Vietoris-Rips complex of the pointcloud formed by the 3 coordinate points (each coordinate being a log return of each index) - computed in the ph_rv_algorithm.py.
...
Due to lack of data (only daily was available), a modification was made to Yang and Zhang's Realized Volatility proxy. In case you have access to higher frequency data, I also leave in this repository a RV_functions.py with functions (H. Souto and Amir Moradi's work), choose which one's best for you. The rest of the code should keep working, as long as you save the RV series of each index and the series of wasserstein distances (it´s the same for all 3 indexes) saved as ph_rv_algorithm.py does.
